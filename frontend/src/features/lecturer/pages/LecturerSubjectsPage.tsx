...
import { Check, X, Filter, Pencil, Trash2 } from "lucide-react";
import {
    deleteProposalByLecturer,
    editApprovedByLecturer,
...
export default function LecturerSubjectsPage() {
    ...
    async function handleDelete(id: string) {
        try {
            await deleteProposalByLecturer(id);
    ...
    async function handleEditSave(payload: Parameters<typeof editApprovedByLecturer>[1]) {
        if (!editing) return;
        setSavingEdit(true);
        try {
            await editApprovedByLecturer(editing.id, payload);
